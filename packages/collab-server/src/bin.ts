...
// >>> AIM-FORK: anonymous mode on a non-loopback bind must be opted into
// explicitly — otherwise the server refuses to START (stricter than upstream's
// warn-only posture). See startup-guard.ts, audit 2026-07-12.
...
async function main() {
  // >>> AIM-FORK: supersedes upstream's warn-only block here. Same diagnosis
  // (no secret + public bind = world-writable server), but we refuse to start
  // instead of logging and continuing, unless COLLAB_ALLOW_ANONYMOUS=1 says the
  // operator meant it. The loopback case still only warns.
