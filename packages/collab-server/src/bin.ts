...
// >>> AIM-FORK: startup posture guard — hard-refuse anonymous mode on a
// non-loopback bind unless explicitly opted in (see startup-guard.ts). Stricter
// than upstream's warn-only path. Kept local; candidate to upstream.
...
// >>> AIM-FORK: explicit opt-in to run anonymous (no token secret) on a
// non-loopback bind. Without this the startup guard hard-exits (see below).
...
async function main() {
  // >>> AIM-FORK: stricter startup posture than upstream's warn-only block.
  // An anonymous (no token secret) server on a non-loopback host is world-
  // writable, so we hard-exit unless COLLAB_ALLOW_ANONYMOUS=1 opts in; loopback
  // and token-secured binds warn/pass exactly as before.
