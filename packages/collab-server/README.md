...
| `COLLAB_ALLOW_ANONYMOUS` | _(unset)_ | `1` = explicitly allow anonymous mode (no `COLLAB_TOKEN_SECRET`) on a non-loopback bind (world-writable — trusted networks only). Without it the server refuses to start in that posture. |
